...
function iconChart() {
  return `
    <svg viewBox="0 0 24 24" class="icon" fill="none" aria-hidden="true">
      ...
      <path d="M8.5 16v-4M12 16V8M15.5 16v-6" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" />
    </svg>
  `;
}
...
function iconRefresh() {
  return `
    <svg viewBox="0 0 24 24" class="icon" fill="none" aria-hidden="true">
      ...
      <path d="M19 12a7 7 0 1 0-2.05 4.95" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" />
    </svg>
  `;
}
...
function dashboardView({ uploads = [], summary = {}, config = {} }) {
  ...
  const medisComposition = total > 0 ? Math.round((medis / total) * 100) : 0;
  const nonMedisComposition = total > 0 ? Math.round((nonMedis / total) * 100) : 0;
  const unknownComposition = total > 0 ? Math.round((unknown / total) * 100) : 0;

  const totalProgress = calculateProgress(total, DATASET_TARGET);
  const medicalProgress = calculateProgress(medis, MEDICAL_TARGET);
  const nonMedicalProgress = calculateProgress(nonMedis, NON_MEDICAL_TARGET);

  const remainingTotal = Math.max(0, DATASET_TARGET - total);
  const remainingMedical = Math.max(0, MEDICAL_TARGET - medis);
  const remainingNonMedical = Math.max(0, NON_MEDICAL_TARGET - nonMedis);

  const latestUpload = uploads[0] || null;
  const averageFileSize = total > 0 ? totalBytes / total : 0;

  const syncReady = config.syncReady === true;
  const syncMessage = config.syncMessage || "Belum ada informasi koneksi.";
  const lastSyncedAt = config.lastSyncedAt ? formatDateTime(config.lastSyncedAt) : "-";
  ...
  const recentRows =
    uploads.length > 0
      ? uploads
          .slice(0, 15)
          .map((item) => {
            const imageUrl = escapeHtml(item.secure_url || "");
            const publicId = escapeHtml(item.public_id || "-");
            const source = escapeHtml(item.source || "esp32cam");
              ...
              <tr>
                <td>
                  <div class="capture-cell">
                    <a class="capture-thumb" href="${imageUrl}" target="_blank" rel="noreferrer">
                      <img src="${imageUrl}" alt="Capture preview" />
                    </a>
                    <div class="capture-info">
                      <div class="capture-name">${publicId}</div>
                      <div class="capture-meta">${source}</div>
                    </div>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <a class="row-link" href="${imageUrl}" target="_blank" rel="noreferrer">
                    Buka ${iconArrow()}
                  </a>
                </td>
              </tr>
      ...
      : `
        <tr>
          <td colspan="5">
            <div class="empty-table">Belum ada citra yang tersimpan.</div>
          </td>
        </tr>
      `;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .sidebar-footer span { display: block; margin-bottom: 6px; color: rgba(255, 255, 255, 0.56); font-size: 12px; }
    .sidebar-footer strong { display: block; font-size: 13px; line-height: 1.4; word-break: break-word; }
    ...
    .page-title h1 { margin: 0; color: var(--text-strong); font-size: 28px; letter-spacing: -0.045em; }
    .page-title p { margin: 6px 0 0; color: var(--muted); font-size: 14px; }
    .top-actions { display: flex; justify-content: flex-end; flex-wrap: wrap; gap: 10px; }
    .btn, .theme-toggle {
      min-height: 42px;
      padding: 0 14px;
      display: inline-flex;
    ...
    .mini-icon { width: 15px; height: 15px; display: inline-block; vertical-align: middle; }
    .metrics {
      display: grid;
    ...
    .metric-storage { font-size: 24px; letter-spacing: -0.04em; }
    .metric-note { min-height: 34px; margin-top: 8px; color: var(--muted-2); font-size: 12px; line-height: 1.45; }
    .progress-box { display: grid; gap: 7px; margin-top: 10px; }
    .progress-track { height: 8px; overflow: hidden; border-radius: 999px; background: var(--gray-soft); }
    .progress-fill { height: 100%; border-radius: 999px; background: var(--accent); }
    .progress-fill-blue { background: var(--blue); }
    .progress-caption { display: flex; justify-content: space-between; gap: 8px; color: var(--muted-2); font-size: 12px; line-height: 1.4; }
    .content-grid {
      display: grid;
    ...
    .panel-link { color: var(--accent); text-decoration: none; font-size: 13px; font-weight: 800; white-space: nowrap; }
    ...
    td { padding: 14px 20px; color: var(--text); border-bottom: 1px solid var(--line); font-size: 14px; vertical-align: middle; }
    tr:last-child td { border-bottom: none; }
    .capture-cell { min-width: 0; display: flex; align-items: center; gap: 12px; }
    .capture-thumb {
      width: 48px;
      height: 48px;
      flex: 0 0 auto;
    ...
    .capture-thumb img { width: 100%; height: 100%; display: block; object-fit: cover; }
    ...
    .capture-meta { margin-top: 3px; color: var(--muted); font-size: 12px; }
    .label { display: inline-flex; padding: 5px 9px; border-radius: 999px; color: #047857; background: var(--accent-soft); font-size: 12px; font-weight: 800; }
    html[data-theme="dark"] .label { color: #5eead4; }
    .label-blue { color: #1d4ed8; background: var(--blue-soft); }
    html[data-theme="dark"] .label-blue { color: #93c5fd; }
    .label-gray { color: #4b5563; background: var(--gray-soft); }
    html[data-theme="dark"] .label-gray { color: #cbd5e1; }
    .row-link { display: inline-flex; align-items: center; gap: 5px; color: var(--accent); text-decoration: none; font-size: 13px; font-weight: 800; }
    .empty-table { padding: 34px 10px; color: var(--muted); text-align: center; font-size: 14px; }
    .side-stack { display: grid; align-content: start; gap: 16px; }
    .target-card, .split-card, .system-card, .latest-card { padding: 20px; }
    .target-list, .split-list, .system-list { display: grid; gap: 14px; margin-top: 16px; }
    .target-item, .split-item { display: grid; gap: 8px; }
    .target-head, .split-head { display: flex; justify-content: space-between; gap: 10px; color: var(--text); font-size: 13px; font-weight: 750; }
    .target-detail { color: var(--muted); font-size: 12px; }
    ...
    .system-item span { color: var(--text); font-size: 13px; font-weight: 650; }
    .system-value {
      max-width: 210px;
      color: var(--muted);
      text-align: right;
    ...
    .status-ok, .status-bad { display: inline-flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 850; }
    ...
    .latest-preview img { width: 100%; height: 100%; display: block; object-fit: cover; }
    .latest-empty {
      margin-top: 14px;
      padding: 28px 16px;
      border-radius: 18px;
  ...
  <div class="app">
    <aside class="sidebar">
      ...
      <div class="sidebar-footer">
        ...
        <strong>${escapeHtml(config.folder || "skripsi_dataset_medis")}</strong>
      </div>
    </aside>
    <main class="main">
      <header class="topbar">
        <div class="page-title">
          ...
          <p>Target 3.000 citra: 1.500 medis dan 1.500 non-medis.</p>
        </div>
      ...
      <section class="metrics">
        <article class="metric-card">
          <div class="metric-top">
            ...
            <div class="metric-icon">${iconTarget()}</div>
          </div>
          <h2 class="metric-value">${total}</h2>
          <div class="metric-note">Target ${DATASET_TARGET} gambar · kurang ${remainingTotal}</div>
        ...
        <article class="metric-card">
          <div class="metric-top">
            <div class="metric-label">Limbah medis</div>
            <div class="metric-icon">${iconCheck()}</div>
          </div>
          <h2 class="metric-value">${medis}</h2>
          <div class="metric-note">Target ${MEDICAL_TARGET} gambar · kurang ${remainingMedical}</div>
          <div class="progress-box">
            <div class="progress-track">
              <div class="progress-fill" style="width: ${medicalProgress}%;"></div>
            </div>
            <div class="progress-caption">
        ...
        <article class="metric-card">
          <div class="metric-top">
            <div class="metric-label">Limbah non-medis</div>
            <div class="metric-icon metric-icon-blue">${iconGrid()}</div>
          </div>
          <h2 class="metric-value">${nonMedis}</h2>
          <div class="metric-note">Target ${NON_MEDICAL_TARGET} gambar · kurang ${remainingNonMedical}</div>
          <div class="progress-box">
            <div class="progress-track">
              <div class="progress-fill progress-fill-blue" style="width: ${nonMedicalProgress}%;"></div>
            </div>
            <div class="progress-caption">
              <span>${nonMedicalProgress}% tercapai</span>
              <span>${nonMedis}/${NON_MEDICAL_TARGET}</span>
            </div>
          </div>
        </article>
        <article class="metric-card">
          ...
          <h2 class="metric-value metric-storage">${formatBytes(totalBytes)}</h2>
          <div class="metric-note">Akumulasi ukuran seluruh gambar yang tercatat.</div>
          <div class="progress-box">
            <div class="progress-track">
              <div class="progress-fill" style="width: ${total > 0 ? 100 : 0}%;"></div>
            </div>
            <div class="progress-caption">
              <span>Rata-rata per gambar</span>
              <span>${formatBytes(averageFileSize)}</span>
            </div>
          </div>
        </article>
      </section>
        ...
        <div class="panel">
          <div class="panel-head">
            <div>
              ...
              <p>15 citra terbaru yang diperoleh dari Cloudinary.</p>
            </div>
            <a class="panel-link" href="/gallery">Lihat semua</a>
          </div>
        ...
        <aside class="side-stack">
          <div class="panel target-card">
            <div class="panel-head" style="padding: 0; border-bottom: none;">
              <div>
                ...
                <p>Kemajuan terhadap target tiap kelas.</p>
              </div>
            </div>
            <div class="target-list">
              <div class="target-item">
                ...
                <div class="track"><div class="fill" style="width: ${totalProgress}%;"></div></div>
                <div class="target-detail">${total} dari ${DATASET_TARGET} gambar, kurang ${remainingTotal}.</div>
              </div>
              <div class="target-item">
                <div class="target-head">
                  <span>Medis</span>
                ...
                <div class="track"><div class="fill" style="width: ${medicalProgress}%;"></div></div>
                <div class="target-detail">${medis} dari ${MEDICAL_TARGET} gambar, kurang ${remainingMedical}.</div>
              </div>
              <div class="target-item">
                <div class="target-head">
                  <span>Non-medis</span>
                  <strong>${nonMedicalProgress}%</strong>
                </div>
                <div class="track"><div class="fill fill-blue" style="width: ${nonMedicalProgress}%;"></div></div>
                <div class="target-detail">${nonMedis} dari ${NON_MEDICAL_TARGET} gambar, kurang ${remainingNonMedical}.</div>
              </div>
            </div>
          </div>
          <div class="panel split-card">
            <div class="panel-head" style="padding: 0; border-bottom: none;">
              <div>
                ...
                <p>Persentase berdasarkan data aktual.</p>
              </div>
            </div>
            <div class="split-list">
              <div class="split-item">
                <div class="split-head">
                  <span>Medis</span>
                  <strong>${medisComposition}%</strong>
                </div>
                <div class="track"><div class="fill" style="width: ${medisComposition}%;"></div></div>
              </div>
              <div class="split-item">
                <div class="split-head">
                  <span>Non-medis</span>
                  <strong>${nonMedisComposition}%</strong>
                </div>
                <div class="track"><div class="fill fill-blue" style="width: ${nonMedisComposition}%;"></div></div>
              </div>
              <div class="split-item">
                <div class="split-head">
                  <span>Unknown</span>
                  <strong>${unknownComposition}%</strong>
                </div>
                <div class="track"><div class="fill fill-gray" style="width: ${unknownComposition}%;"></div></div>
              </div>
            </div>
          </div>
          <div class="panel system-card">
            <div class="panel-head" style="padding: 0; border-bottom: none;">
              <div>
                <h2>System</h2>
                <p>Status backend dan koneksi Cloudinary.</p>
              </div>
            </div>
            <div class="system-list">
              ...
              <div class="system-item">
                ...
                ${
                  syncReady
                    ? `<strong class="status-ok">${iconCheck()} Cloudinary terhubung</strong>`
                    : `<strong class="status-bad">Tidak terhubung</strong>`
                }
              </div>
              <div class="system-item">
                <span>Status</span>
                <strong class="system-value ${syncReady ? "status-ok" : "status-bad"}">
                  ${escapeHtml(syncMessage)}
                </strong>
              </div>
              <div class="system-item">
                <span>Terakhir dimuat</span>
                <strong class="system-value">${escapeHtml(lastSyncedAt)}</strong>
              </div>
              <div class="system-item">
          ...
          <div class="panel latest-card">
            <div class="panel-head" style="padding: 0; border-bottom: none;">
              <div>
                ...
                <p>Citra terbaru yang masih tersedia.</p>
              </div>
            </div>
            ${
              latestUpload
                ? `
                  <a class="latest-preview" href="${escapeHtml(latestUpload.secure_url || "")}" target="_blank" rel="noreferrer">
                    <img src="${escapeHtml(latestUpload.secure_url || "")}" alt="Latest capture" />
                  </a>
                `
                : `<div class="latest-empty">Belum ada preview.</div>`
            }
          </div>
        </aside>
      </section>
